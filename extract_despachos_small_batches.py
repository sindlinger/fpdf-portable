#!/usr/bin/env python3
"""
Script para extrair despachos de 2024 em lotes pequenos para evitar timeout
"""
import subprocess
import json
import re
...
def parse_fpdf_output(output_text):
    ...
    for line in output_text.split('\n'):
        ...
        # Skip info lines and separators
        if (line.startswith('[INFO]') or line.startswith('Finding PAGES') or 
            line.startswith('Active filters') or line.startswith('🔍') or 
            ...
            if 'arquivo' in data and 'paginas' in data:
                ...
                for page in data['paginas']:
                    despacho_info = {
                        ...
                        'conteudo': page['content'][:500] + '...' if len(page['content']) > 500 else page['content'],
...
def main():
    """Main function to extract despachos from 2024 files in small batches"""
    print("=== Extração de Despachos 2024 (Lotes Pequenos) ===")
    print(f"Iniciado em: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Process in smaller batches of 10 to avoid timeouts
    batch_size = 10
    start_idx = 2  # First 2024 file based on cache list
    end_idx = 100   # First 100 files to test
    ...
    for batch_start in range(start_idx, end_idx + 1, batch_size):
        ...
        if current_batch % 5 == 0:
            ...
            intermediate_file = f"despachos_partial_{timestamp}.json"
            ...
            print(f"Resultados parciais salvos em: {intermediate_file}")
    
    # Save final results
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    results_file = f"despachos_2024_test_{timestamp}.json"
